            ...
            <div className="flex flex-col px-14 md:px-0 space-y-40">
                {
                    projects.map((project, index) => {
                        return (
                            <div key={index} className={`flex ${index % 2 === 0 ? 'md:flex-row' : 'md:flex-row-reverse'} flex-col flex-wrap md:justify-evenly`}>
                                <div className="flex flex-col mt-10 md:mt-0 items-start">
                                    <div className='flex flex-col align-left my-2'>
                                        ...
                                        <p className='text-4xl font-medium text-purple-800'>{project.name}</p>
                                    </div>
                                    ...
                                    <div className='flex flex-col py-2 items-start'>
                                        ...
                                        <div className='flex flex-wrap'>
                                            {project.techStack.map((tech, idx) => {
                                                return (
                                                    <span key={idx} className='text-sm text-custom-purple-400 px-1'>{tech}</span>
                                                )
                                            })}
                                        </div>
                                    </div>
                                ...
                                <div className="flex md:w-[35rem] md:h-[22rem] overflow-hidden duration-700 rounded-lg shadow-xl hover:shadow-2xl">
                                    <Image src={project.image} objectFit='contain' alt={project.name} />
                                </div>
                            </div>
                        )
                    })
